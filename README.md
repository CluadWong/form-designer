...
[![npm version](https://img.shields.io/npm/v/@cluadwong/form-designer)](https://www.npmjs.com/package/@cluadwong/form-designer)
[![license](https://img.shields.io/npm/l/@cluadwong/form-designer)](./LICENSE)
...
- `main`：稳定分支，跟踪 npm 上的已发布版本。
- `release`：发布分支，`npm run release` 在此运行（升版本 → 构建 → 发布 npm → 推 release + tags）。
- `dev`：开发主线，功能改动先进这里，再同步到 `release`。
